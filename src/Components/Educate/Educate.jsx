...
export default function VerticalLinearStepper() {
  ...
  const langSelect=(event)=>{
    let hun = false;
    let eng = true;
    let clickedFlag=event.target.id;
    if  (clickedFlag=="hun"){
      let hun = true;
      let eng = false;
      //display tulajdonságot kell billenteni, ha hun true és fordítva
      <Box sx={{ maxWidth: 800 }}>
      <Stepper activeStep={activeStep} orientation="vertical">
        {steps.map((stepH, index) => (
          <Step key={stepH.label}>
            <StepLabel className='stepDiv'>
              {stepH.label}
            </StepLabel>
            <StepContent>              
                <Typography className='descript'>{stepH.description}</Typography>                                         
              <Box sx={{ mb: 2 }}>
                <div>
                  <Button
                    variant="contained"
  ...
  return (
    ...
    <section className='eduStep container-fluid'>
      <div className='row-col-2 col-4'>
          <img className='cvFlag' onClick={langSelect} src={Hunflag}></img> |
